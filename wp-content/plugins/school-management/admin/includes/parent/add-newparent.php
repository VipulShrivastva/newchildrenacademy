...
$(document).ready(function() {
	...
	$('#add-another_item').live('click',function(event) {
	...
	$('#revove_item').live('click',function(event) {
		event.preventDefault();
		var numItems = $('.parents_child').length;
			...
			<div class="col-sm-8">
				<?php if($edit){ $relationval=$user_info->relation; }elseif(isset($_POST['relation'])){$relationval=$_POST['relation'];}else{$relationval='';}?>
                     <select name="relation" class="form-control validate[required]" id="relation">
                        ...
                        <option value="Father" <?php selected( $relationval, 'Father'); ?>><?php _e('Father','school-mgt');?></option>
                        <option value="Mother" <?php selected( $relationval, 'Mother'); ?>><?php _e('Mother','school-mgt');?></option>
                     </select>
			</div>	
		</div>
		<div class="form-group">
