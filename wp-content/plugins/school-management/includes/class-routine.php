...
class Class_routine
{
	...
	public function is_route_exist($route_data)
	{
		...
		$route =$wpdb->get_row("SELECT * FROM $table_name WHERE subject_id=".$route_data['subject_id']." AND teacher_id=".$route_data['teacher_id']." AND class_id=".$route_data['class_id']." AND start_time='".$route_data['start_time']."' AND end_time='".$route_data['end_time']."' AND weekday=".$route_data['weekday']);
		if(empty($route))
			return true;
		else
			return false;
	...
	public function get_periad($class_id,$week_day)
	{
		global $wpdb;
		$table_name = $wpdb->prefix . $this->table_name;
