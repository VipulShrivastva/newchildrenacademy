...
function smgt_get_user_notice($role,$class_id)
{
	if($role == 'all' )
	{
		$userdata = array();
...
function get_payment_report($class_id,$fee_term,$payment_status,$year)
{
	
	$year_year = explode('-',$year);
	$where = '';
	$array_test = array();
	if($class_id != ' ')
...
function get_subject_name_by_teacher($teacher_id)
{
	global $wpdb;
	$table_name = $wpdb->prefix . "subject";
	return $retrieve_subject = $wpdb->get_var( "SELECT sub_name FROM $table_name WHERE teacher_id=".$teacher_id);	
...
function get_send_message($user_id,$max=10,$offset=0)
{
	...
	if(is_admin() || $class_obj->role=='teacher' || $class_obj->role=='parent')
	{
...
<br /><a href="<?php echo wp_logout_url(); ?>"><i class="fa fa-sign-out m-r-xs" /> Logout</a> 
<?php 
	 }
	 else 
	 {
...
function smgt_accept_return_book()
{
	...
	$booklist = $wpdb->get_results("SELECT * FROM $table_issuebook where student_id=$stud_id and status='Issue'".$id);
...
function smgt_load_user()
{
		...
		echo  $class_id =$_POST['class_list'];
...
<div class="panel-body">
  ...
  <table class="table table-bordered" border="1">
    ...
    <tr>
      ...
      <td><?php echo $parent->relation; echo $parent->photo;?></td>
    </tr>
    <?php
					}
				}
						...
						<tbody>
							<tr>
								...
								<td align="right" width="24%">
									...
									<h5><?php echo __('Status','school-mgt')." : ".$payment_status;?></h5>
								</td>
							</tr>
						</tbody>
					</table>
...
function smgt_student_view_paymenthistory()
{
	...
	<div id="invoice_print"> 
		<table width="100%" border="0">
					...
					echo get_payment_status($fees_detail_result->fees_pay_id);
					echo "</span>";?></h5>
								</td>
							</tr>
						</tbody>
...
function smgt_add_remove_feetype()
{
	...
	smgt_add_category_type($model);
}
function smgt_add_category_type($model) 
{
	$title = "Title here";
